...
class EvidenceAgent:
    ...
    def analyze(self, context: TraceContext) -> EvidenceReview:
        ...
        valid_event_ids = set(context.related_event_ids)
        valid_detection_ids = set(context.related_detection_ids)
        valid_edge_ids = set(context.edge_ids)
        for review in result.stages:
            review.event_ids = [
                item for item in review.event_ids if item in valid_event_ids
            ]
            review.detection_ids = [
                item for item in review.detection_ids if item in valid_detection_ids
            ]
            review.edge_ids = [
                item for item in review.edge_ids if item in valid_edge_ids
            ]
            validate_subset(review.event_ids, valid_event_ids, "event_ids")
            validate_subset(review.detection_ids, valid_detection_ids, "detection_ids")
            validate_subset(review.edge_ids, valid_edge_ids, "edge_ids")
        return result
