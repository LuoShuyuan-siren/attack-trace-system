...
def test_sysmon_12_registry_persistence_event():
    event = parser._parse_event(
        12,
        {
            "Image": r"C:\Windows\System32\reg.exe",
    ...
    assert event is not None
    assert event.event_type == "registry_modify"
    assert event.action == "create_registry"
    assert event.object.path.endswith("Updater")


def test_sysmon_23_file_delete_event():
    event = parser._parse_event(
        23,
        {
            "Image": r"C:\Windows\System32\cmd.exe",
            "ProcessId": "3000",
            "TargetFilename": r"C:\Users\Public\payload.exe",
        },
        TIMESTAMP,
        HOSTNAME,
    )
    ...
    assert event is not None
    assert event.event_type == "file_delete"
    assert event.object.path.endswith("payload.exe")
