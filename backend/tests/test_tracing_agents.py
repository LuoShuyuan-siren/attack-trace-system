...
def test_hallucinated_detection_ids_are_clipped_before_validation():
    evidence = EvidenceReview(stages=[
        StageEvidenceReview(
            stage="initial_access", status="supported", supported=True,
            confidence=0.9, event_ids=["evt-initial"],
            detection_ids=["initial", "det-fake-not-in-context"],
            edge_ids=[], explanation="supported",
        )
    ], overall_confidence=0.9)
    chain_review = ChainReview(
    ...
    bad_report = TraceReport(
        attack_origin="ip:203.0.113.8", victim_hosts=[],
        privilege_escalation="not evidenced", c2_communication="not evidenced",
        data_exfiltration="not evidenced", techniques=["T1190"],
        key_evidence_ids=["initial"], overall_confidence=0.9,
        timeline=[], uncertainties=[], summary="structured report",
    )
    fake = FakeLLM({
        EvidenceReview: evidence,
        ChainReview: chain_review,
        AttributionReview: attribution,
        TraceReport: bad_report,
    })
    result = TraceAgentOrchestrator(fake).analyze([], chain())
    assert result["agent_status"]["degraded"] is False
    assert result["agent_analysis"]["evidence"]["stages"][0]["detection_ids"] == ["initial"]
